...
const middleware = {
	...
	async searchPostCateFilter(req, res, next){
		...
		if(queryKeys.length){
			...
			console.log(res.locals.dbQueries);
		}
	...
	async searchAndFilterPosts(req, res, next) {
		...
		if(queryKeys.length) {
			...
			if (price) {
				if (price.min) dbQueries.push({ price: { $gte: "10" } });
				if (price.max) dbQueries.push({ price: { $lte: "1000" } });
			}
